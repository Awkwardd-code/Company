...
function RightSidebar({ variant = 'default' }: RightSidebarProps) {
  ...
  return (
    <>
      ...
      {/* Sidebar Content - Only renders when sidebarOpen is true */}
      {sidebarOpen && (
        <div
          id="sidebar"
          ref={sidebar}
          className={`flex flex-col fixed z-50 right-0 top-0 h-[100dvh] bg-white dark:bg-gray-800 p-4 transition-transform duration-200 ease-in-out overflow-hidden
            ${sidebarExpanded ? 'translate-x-0 w-64' : 'translate-x-full w-0'} 
            ${variant === 'v2' ? 'border-l border-gray-200 dark:border-gray-700/60' : 'rounded-l-2xl shadow-xs'}`}
        >
